...
<c:if test="${editNewsVo==null}">
    <c:redirect url="/news/load/viewNews/${param.newsId}"/>
</c:if>
<div class="w1200 pt20 clearfix" style="margin-left: 150px">
    <div class="detail_left fl">
        <ul class="clearfix title">
            <li>
                <c:if test="${editNewsVo.newsRecord.aduitResult==1}">校验新闻</c:if>
                <c:if test="${editNewsVo.newsRecord.aduitResult == 2||editNewsVo.newsRecord.aduitResult == 3||editNewsVo.newsRecord.aduitResult == 4||editNewsVo.newsRecord.aduitResult == 5}">
                查看新闻
                </c:if>
        </ul>
        <div class="artical">
            <h1><a href="javascript:;">${editNewsVo.news.newsTitle}</a></h1>
            <div class="clearfix">
                <ul class="fl">
                    <li>来源：${editNewsVo.news.newsFrom}&nbsp;&nbsp;编辑：${editNewsVo.userName}&nbsp;&nbsp;
                        状态： <c:if test="${editNewsVo.newsRecord.aduitResult==1}">审核中&nbsp;&nbsp;<fmt:formatDate
                                value="${editNewsVo.news.createTime}" pattern="yyyy-MM-dd HH:mm:ss"></fmt:formatDate>(时间为新闻创建时间)</c:if>
                        <c:if test="${editNewsVo.newsRecord.aduitResult==2}">等待发布&nbsp;&nbsp;距离发布还有：<span id="release"
                                                                                                          style="color: #2577E3"></c:if></span>
                    </li>
                </ul>
            </div>
            <div class="abstract">
                摘要：${editNewsVo.news.newsTitle}
            </div>
...
<c:if test="${editNewsVo.newsRecord.aduitResult==1}">
...
<c:if test="${editNewsVo.newsRecord.aduitResult == 2}">
...
<c:if test="${editNewsVo.newsRecord.aduitResult == 3}">
    <shiro:hasRole name="admin">
        <div class="layui-form-item" style="float: right;margin-right: 50px">
...
<c:if test="${editNewsVo.newsRecord.aduitResult == 4}">
    <shiro:hasRole name="admin">
        <div class="layui-form-item" style="float: right;margin-right: 50px">
...
<c:if test="${editNewsVo.newsRecord.aduitResult == 5}">
...
<script type="text/javascript">
    layui.use(['util', 'laydate', 'layer'], function () {
        ...
        var d = new Date("<fmt:formatDate value="${editNewsVo.news.releaseTime}" pattern="yyyy-MM-dd HH:mm:ss"></fmt:formatDate>");
    ...
    $("#issue").click(function () {
        parent.layer.open({
            ...
            content: ['back/manage/news/news-issue?newsId=${editNewsVo.news.newsId}', 'no'],//no表示禁用滚动条
            success: function (layero, index) {
                ...
                $.ajax({
                    type: "POST",
                    url: "news/getNewsStauts?newsId=${editNewsVo.news.newsId}",
                    async: false,
